...
$url = new moodle_url('/course/editcategorySchool.php');
if ($id) {
...
$mform = new core_course_editcategorySchool_form(null, array(
    'categoryid' => $id,
...
$manageurl = new moodle_url('/local/subject/');
if ($mform->is_cancelled()) {
    if ($id) {
        $manageurl = new moodle_url('/local/subject');  
        $parentid = $category->parent;
        $manageurl->param('parent', $parentid);
    } else if ($parent) {
        $manageurl = new moodle_url('/local/subject/index.php');
        $manageurl->param('parent', $parent);
    }
    // $manageurl1 = new moodle_url('/local/subject');
    redirect($manageurl);
