...
function display_category_tree_with_depth($tree) {
    ...
    if (!empty($tree['courses'])) {
        ...
        foreach ($tree['courses'] as $course) {
            echo '<li>' . str_repeat('&nbsp;', $course['depth'] * 4) . '<a class="form-control" href="' . $CFG->wwwroot . '/course/view.php?id=' . $course['id'] . '"><i class="ml-2 mr-2 fas fa-book-open"></i> ' . $course['fullname'] . '</a></li>';
        }
        echo '</ul>';
    } 
...
foreach($schoolids as $categoryid){
    $ids = $categoryid->schoolid;
    $tree = get_category_tree_with_depth($ids);
}
...
<?php
    foreach ($schoolids as $categoryid) {
        $tree = get_category_tree_with_depth($categoryid->schoolid);
