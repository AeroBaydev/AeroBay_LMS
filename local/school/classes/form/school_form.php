...
class school_form extends moodleform
{
    ...
    //     document.getElementById("school_sortname").onchange = function() {
...
foreach ($categories as $category) {
    ...
    public function validation($data, $files) {
        ...
        if (!empty($data['school_sortname'])) {
            if ($existing = $DB->get_record('school', array('school_sortname' => $data['school_sortname']))) {
                if (!$data['id'] || $existing->id != $data['id']) {
                    $errors['school_sortname'] = get_string('school_sortname_error', 'local_school');
                }
            }
        }
        return $errors;
