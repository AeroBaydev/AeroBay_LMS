...
class edit_school_form extends moodleform
{
    ...
    public function definition()
    {
        ...
        // $heading_text = "Update School Details";
        ...
        $mform->addElement('select', 'selectsubcategory', get_string('selectsubcategory', 'local_school'), $optionSubCat, [
            'multiple' => 'multiple', 
        ...
        SELECT ccc.*
        FROM {course_categories} cc
        JOIN {school} s ON cc.idnumber = s.school_id
        JOIN {course_categories} ccc ON ccc.parent = cc.id
        WHERE s.id = :school_id";
        $school_categories = $DB->get_records_sql($sql, array('school_id' => $this->schoolid));
    $optionSubCatSchool=[];
    ...
    $optionSubCatselected=[];
        ...
        $selectedIds= array_keys($optionSubCatselected);
        $mform->setDefault('selectsubcategory', $selectedIds); 
