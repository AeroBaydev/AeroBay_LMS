...
$PAGE->set_title("Assign ARM's to RM");
// $PAGE->navbar->add("Assign ARM's to RM", new moodle_url('/assign_arm_to_rm.php'));
$PAGE->set_heading("Assign ARM's to RM");

// $rmid = $_POST['rmid'];   
$userid = optional_param('userid', 0, PARAM_INT);;   
$PAGE->navbar->add('RM/ARM Management', "$CFG->wwwroot/local/regionalpoc/rm_arm_manage.php?roleid=12");
$PAGE->navbar->add("Assign ARM's to RM", "$CFG->wwwroot/local/regionalpoc/assign_arm_to_rm.php?rmid=$rmid");
    ...
    if (isset($_POST['add'])) {
        ...
        if (isset($_POST['addselect'])) {
            ...
            foreach ($selected_users as $armid) {
                ...
                $DB->execute("INSERT INTO {schoolassign} (schoolid,schoolassignee, schoolassignedto,status) VALUES (?,?,?,1)", [$armid, $userid,$USER->id]);
                
                // Update regionalpoc table
                // $DB->execute("UPDATE {regionalpoc} SET status = 1 WHERE id = ? AND role = 'asstmanager'", [$armid]);
            }
        }
        ...
        if (isset($_POST['removeselect'])) {
            ...
            foreach ($selected_users as $armid) {
                ...
                // $DB->execute("DELETE FROM {schoolassign} WHERE schoolassignee = ? AND schoolassignedto = ? AND", [$userid, $armid]);
                $DB->execute("UPDATE {schoolassign} SET status = 0 WHERE schoolassignedto = ? AND schoolid = ? AND schoolassignee=?", [$userid, $armid,$USER->id]);
            }
        }
    }
}
...
// $users = $DB->get_records_sql("SELECT id, username FROM {regionalpoc} WHERE role='asstmanager' AND status=0 AND pocid = $USER->id");
...
$users = $DB->get_records_sql("SELECT ass.id, ass.school_name as username FROM {school} as ass join {schoolassign} as asa on ass.course_cat_id=asa.schoolid WHERE asa.schoolassignee = 228 and asa.status = 0");
$existing_users = $DB->get_records_sql("SELECT ass.id, ass.school_name as username FROM {school} as ass join {schoolassign} as asa on ass.course_cat_id=asa.schoolid WHERE schoolassignee = 228");


$mform->display();
?>
<style>
    a.backbutton.btn.btn-primary {
    margin-left: 53px;
    }

</style>
<form method="post" action="">
    <input type="hidden" name="userid" value="<?php  echo $rmid; ?>" id="userid">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // console.log(rmid)
    fetchExistingUsers(userid);

    addBtn.addEventListener('click',()=>{
        userid.setAttribute('value',roleSelect.value);
    })
    ...
    function fetchExistingUsers(roleId) {
        ...
        xhr.open('POST', 'fetch_school.php', true);
