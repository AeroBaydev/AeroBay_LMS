...
class student_form extends moodleform {
    ...
    public function definition() {
      ...
      $userid=  $USER->id;
        ...
        if($gradeid){
         ...
         //   $course_list= $DB->get_records_sql_menu("SELECT cc.id, cc.fullname FROM {course} cc where cc.category=$gradeid");
         }

        // Adding custom heading
        // $heading_text = "Add New Student";
        ...
        $mform->addElement('select', 'gradeid', get_string('selectgrade', 'local_students'), array('' => get_string('selectgrade', 'local_students'))+ $gradeid_list);
        $mform->setType('gradeid', PARAM_INT);
        $mform->addRule('gradeid', get_string('required'), 'required', null, 'client');

        // $mform->addElement('select', 'courseid', get_string('selectcourse', 'local_students'), array('' => get_string('selectcourse', 'local_students'))+ $course_list);
        // $mform->setType('courseid', PARAM_INT);
        // $mform->addRule('courseid', get_string('required'), 'required', null, 'client');

        // $mform->addElement('select', 'sectionid', get_string('selectsection', 'local_students'), array('' => get_string('selectsection', 'local_students')));
    ...
    function validation($data, $files) {
        ...
        if ($DB->record_exists('user', ['username' => $data['username']])) {
            $errors['username'] = get_string('userexists', 'local_students');
        }

        if (!validate_email($data['email'])) {
            $errors['email'] = get_string('invalidemail', 'local_students');
        }
