...
$url = new moodle_url('/local/subject/managementSchool.php');
    ...
    switch ($action) {
        ...
        case 'deletecategory':
            ...
            if ($mform->is_cancelled()) {
                $manageurl = new \moodle_url('/local/subject/index.php', array('id' => $categoryid));
                $category = core_course_category::get($categoryid);
                $parent_id = $category->parent;
                if($parent_id){
                    $manageurl = new \moodle_url('/local/subject/index.php', array('parent' => $parent_id));
            ...
            if ($data = $mform->get_data()) {
                ...
                if ($data->fulldelete == 1 && $category->can_delete_full()) {
                    ...
                    $manageurl = new \moodle_url('/local/subject/index.php', array('parent' => $_SESSION['parentid']));
               
                
                   // $continueurl = new moodle_url('/local/subject/', array('categoryid' => $data->newparent));
                    echo $renderer->continue_button($manageurl);
...
// Assuming your <select> element ID is 'id_fulldelete'
var selectElement = document.getElementById('id_fulldelete');
// Set the value of the second option (index 1) to '1'
