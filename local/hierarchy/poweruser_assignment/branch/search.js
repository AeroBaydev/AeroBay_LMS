...
$(document).ready(function() {
	...
	function search(param1) { //param1 will be 'potential' or 'existing' 
		...
		var request = $.ajax({ //get data through ajax 
			url: 'search.php',
			type: 'POST',
			data: { type: param1, branchid: branch_id, searchtext: $("#id_"+param1+"_searchtext").val() }
		...
		request.done(function(data) { //on success 
			...
			if(data==0) { //no records match the search 
				...
				$("#id_"+param1+"_optgroup").append("<option disabled='disabled'> No users found </option>"); //option showing no courses 
				...
				for(var i=0; i<arrayObj.length; i++) { //append the options from the array 
					$("#id_"+param1+"_optgroup").append("<option value='" + arrayObj[i].id + "'>" + arrayObj[i].firstname + " " + arrayObj[i].lastname + " (" +arrayObj[i].email + ") </option>");
				}
