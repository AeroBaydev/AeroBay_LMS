...
$query="SELECT * FROM mdl_custom_tenants where status=1  and cronstatus=0 limit 0,1";
...
// $tin2 = mysqli_query($conn,$query2);
 ...
 while($row=mysqli_fetch_object($tin )) {
$query1="update mdl_custom_tenants set cronstatus=1 where  id=".$row->id;
 $tin1 = mysqli_query($conn,$query1);
$_SERVER['HTTP_HOST']=$row->subdomain.'.vaidusi.in';
require('/var/www/html/vaidusi/config.php');
require_once('/var/www/html/vaidusi/lib/phpexcel/PHPExcel.php');
ini_set('mysql.connect_timeout', 14400000);
ini_set('default_socket_timeout', 1440000);
//require_login();
global $CFG, $DB, $USER; 
$USER->id=6;
$CFG->prefix='tenant24_';
...
FROM {user} AS u INNER JOIN {user_enrolments} AS ue ON u.id=ue.userid INNER JOIN {enrol} AS e ON ue.enrolid=e.id INNER JOIN {course} AS c on e.courseid=c.id INNER JOIN {course_categories} as cc on c.category=cc.id WHERE 1=1  GROUP BY u.id, c.id ORDER BY u.id ASC limit 0,9000000 ";
...
$save_name='User Report.xlsx';
$objWriter->save('/var/www/html/vaidusi/local/hierarchy/reports/user_'.$row->subdomain.'.xlsx');
