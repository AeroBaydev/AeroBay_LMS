...
path - /user/editlib.php 

custom folder - js

custom file - fetch.php 

search 'chetan_edit' to see changes 
...
function useredit_shared_definition(&$mform, $editoroptions, $filemanageroptions, $user) {
	...
	//chetan_edit 
		...
		$options = array(''=>'Select branch') + $result;
		...
		$mform->addElement('select', 'branch', 'Branch', $options, array('id'=>'id_branch'));
		$mform->addElement('select', 'dept', 'Department', $options2, array('id'=>'id_dept'));
		// $mform->disabledif('dept', 'branch', 'eq', '');
	
		$modulejs=array(
		'name'=>'apply',
		'fullpath'=>'/user/js/selection.js',
		'string'=>array(),
		'requires' => array(),
		);
		if($user->id != -1)
		...
		$PAGE->requires->js_init_call('apply',array($userid, $dept, "id_branch", "id_dept"),true,$modulejs);
